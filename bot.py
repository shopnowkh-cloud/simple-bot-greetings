...
async def send_photo(chat_id: int | str, photo: bytes, caption: str | None = None, reply_markup: dict | None = None) -> dict | None:
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            res = await client.post(f"{TELEGRAM_API_BASE}/bot{token}/sendPhoto", data=data, files=files)
            resp = res.json()
...
async def send_document(chat_id: int | str, buffer: bytes, filename: str, caption: str | None = None) -> dict | None:
    try:
        token = get_bot_token()
        async with httpx.AsyncClient(timeout=60) as client:
            files = {"document": (filename, buffer, "text/plain")}
            data: dict = {"chat_id": str(chat_id), "parse_mode": "HTML"}
            if caption:
                data["caption"] = caption
            res = await client.post(f"{TELEGRAM_API_BASE}/bot{token}/sendDocument", data=data, files=files)
            resp = res.json()
            if not res.is_success or not resp.get("ok"):
...
def now_kh() -> str:
    ...
    t_kh = t + timedelta(hours=7)
    return t_kh.strftime("%Y-%m-%d %H:%M:%S") + " +07"

def now_kh_file() -> str:
    t = datetime.now(timezone.utc)
    from datetime import timedelta
    t_kh = t + timedelta(hours=7)
    return t_kh.strftime("%Y%m%d%H%M%S")
...
def format_account(acc: Any) -> str:
    ...
    if isinstance(acc, dict):
        if acc.get("email"):
            return acc["email"]
        if acc.get("phone"):
            return f"{acc['phone']} | {acc.get('password', '')}"
        if acc.get("code"):
            return acc["code"]
...
async def create_khpay_payment(token: str, amount: float) -> dict:
    try:
        ...
        if qr_string:
            try:
                img_buffer = generate_plain_qr(qr_string)
            except Exception:
                img_buffer = None
...
async def check_khpay_status(token: str, transaction_id: str, md5: str | None = None) -> dict:
    try:
        check_md5 = md5 or transaction_id
        data = await cambo_request(token, {"type": "check_md5", "md5": check_md5}, timeout_ms=15000, retries=1)
        status = str(data.get("status", "")).lower()
        paid = status in ("paid", "success", "completed")
        return {"paid": paid, "status": status or "pending", "data": data}
    except Exception as e:
...
async def load_db() -> dict:
    try:
        ...
        return {
            "accounts": {
                "account_types": v.get("accounts", {}).get("account_types", {}),
                "prices": v.get("accounts", {}).get("prices", {}),
            },
...
def load_ctx(db: dict) -> dict:
    ...
    if ea:
        try:
            extra_admins = set(map(int, json.loads(ea)))
        except Exception:
            pass
    return {
        "db": db,
...
async def main_kb(ctx: dict, uid: int) -> dict:
    if is_admin(ctx, uid):
        return ADMIN_SETTINGS_KB
    return REMOVE_KB

def type_from_cb_id(ctx: dict, cid: str) -> str | None:
    for t in ctx["db"]["accounts"]["account_types"]:
        if type_callback_id(t) == cid:
            return t
    return None
...
async def show_account_selection(ctx: dict, chat_id: int) -> None:
    available = [
        {"at": at, "count": len(v)}
        for at, v in ctx["db"]["accounts"]["account_types"].items()
        if len(v) > 0
    ]
    if not available:
...
async def start_payment_for_session(ctx: dict, chat_id: int, user_id: int, session: dict, cb_id: str | None = None) -> bool:
    ...
    if len(pool) < qty:
        if cb_id:
            await answer_callback_query(cb_id, f"សូមអភ័យទោស! មានត្រឹមតែ {len(pool)} គូប៉ុង នៅក្នុងស្តុក", True)
    ...
    if cb_id:
        await answer_callback_query(cb_id, "កំពុងបង្កើត QR...")

    session["state"] = "payment_pending"
    result = await create_khpay_payment(ctx["CAMBO_API_TOKEN"], session["total_price"])
    img_buffer = result.get("img_buffer")
...
async def deliver_accounts(ctx: dict, chat_id: int, user_id: int, session: dict, payment_data: dict | None = None) -> None:
    ...
    for key in ["photo_message_id", "qr_message_id"]:
        mid = session.get(key)
        if mid:
            asyncio.create_task(delete_message(chat_id, mid))
    delivered = None
    reserved = session.get("reserved_accounts", [])
    if len(reserved) >= qty:
        delivered = reserved[:qty]
    elif len(ctx["db"]["accounts"]["account_types"].get(at, [])) >= qty:
    ...
    for i, acc in enumerate(delivered):
        is_last = i == len(delivered) - 1
        msg = f"🎉 <b>ការទិញបានបញ្ជាក់ដោយជោគជ័យ</b>\n\nគូប៉ុងរបស់អ្នក៖ 👇\n\n<code>{esc(format_account(acc))}</code>\n\n<i>សូមអរគុណសម្រាប់ការទិញ 🙏</i>"
        kb_ = await main_kb(ctx, user_id) if is_last else None
        await send_message(chat_id, msg, kb_)
    try:
        pd = payment_data or {}
        ...
        admin_msg = (
            ...
            f"📦 <b>ប្រភេទ:</b> {esc(at)} × {qty}\n"
            f"💵 <b>ទឹកប្រាក់:</b> ${session['total_price']}\n"
            f"👤 <b>ពីធនាគារ:</b> <code>{esc(from_acc)}</code>\n"
            f"📝 <b>ចំណាំ:</b> {esc(memo)}\n"
            f"🧾 <b>លេខយោង:</b> <code>{esc(ref)}</code>\n"
            f"⏰ <b>ម៉ោង:</b> {now_kh()}"
        )
...
async def run_watchdog(pre_ctx: dict | None = None) -> None:
    ...
    for uid_str, sess in pending:
        ...
        if elapsed >= PAYMENT_TIMEOUT_SEC * 1000:
            ...
            if reserved and at:
                db["accounts"]["account_types"][at] = reserved + db["accounts"]["account_types"].get(at, [])
            db["sessions"].pop(uid_str, None)
            dirty = True
            if sess.get("photo_message_id"):
                asyncio.create_task(delete_message(user_id, sess["photo_message_id"]))
            ...
            continue
        if not sess.get("transaction_id"):
            continue
        try:
            ...
            if not cur or cur.get("transaction_id") != sess.get("transaction_id") or cur.get("state") != "payment_pending":
                continue
...
async def export_stock(ctx: dict, chat_id: int) -> None:
    ...
    if not names:
        ...
        return
    total_avail = sum(len(types.get(t, [])) for t in names)
    W = 60
    lines = [
        "=" * W,
        "  ស្តុក គូប៉ុង / COUPON STOCK".ljust(W),
        f"  {now_kh()}".ljust(W),
        f"  ប្រភេទ: {len(names)}  |  សរុប: {total_avail} គូប៉ុង".ljust(W),
        "=" * W, "",
    ]
    for t in names:
        pool = types.get(t, [])
        price = prices.get(t, 0)
        lines.append(f"[ {t} ]  💰 ${price}  📦 {len(pool)} គូប៉ុង")
        lines.append("─" * W)
        if pool:
            for i, acc in enumerate(pool):
                lines.append(f"  {i+1}. {format_account(acc)}")
        else:
            lines.append("  (គ្មានក្នុងស្តុក)")
        lines.append("")
    lines.append("=" * W)
    buf = "\n".join(lines).encode("utf-8")
    await send_document(chat_id, buf, f"stock_{now_kh_file()}.txt", f"📦 <b>ស្តុក គូប៉ុង</b> — {len(names)} ប្រភេទ, {total_avail} នៅសល់")
    ...
    for uid, info in grouped.items():
        ...
        for i, p in enumerate(info["purchases"]):
            ...
            for a in (p.get("accounts") or []):
                lines.append(f"        • {format_account(a)}")
            if not (p.get("accounts") or []):
                lines.append("        (none)")
    lines += ["", "=" * W, "=" * W]
    buf = "\n".join(lines).encode("utf-8")
    await send_document(chat_id, buf, f"buyers_{now_kh_file()}.txt", f"📋 របាយការណ៍ទិញ — {len(grouped)} អ្នក​ទិញ")
    await send_admin_settings_menu(ctx, chat_id)
...
async def show_users_list(ctx: dict, chat_id: int) -> None:
    ...
    buf = "\n".join(lines).encode("utf-8")
    await send_document(chat_id, buf, f"users_{now_kh_file()}.txt", f"👥 បញ្ជីអ្នកប្រើប្រាស់ — {len(rows)} នាក់")
    await send_admin_settings_menu(ctx, chat_id)

async def send_khpay_info(ctx: dict, chat_id: int) -> None:
    token = ctx["CAMBO_API_TOKEN"]
    if token:
        short = f"<code>{esc(token[:16])}…{esc(token[-4:])}</code>"
    else:
        short = "❌ មិនទាន់កំណត់"
    lines = [
...
async def run_broadcast(ctx: dict, admin_chat_id: int, bcast_text: str) -> None:
    ...
    for uid_str in uids:
        uid = int(uid_str)
        r = await send_message(uid, bcast_text)
        if r:
            sent += 1
        else:
            blocked += 1
...
async def dispatch_admin_button(ctx: dict, chat_id: int, uid: int, btn: str) -> None:
    ...
    if btn == BTN_ADD_ACCOUNT:
        ...
        if not types:
            await send_message(chat_id, "⚠️ <b>មិនមានប្រភេទ គូប៉ុង ណាមួយទេ!</b>")
            return
        labels_map: dict = {}
        rows = []
        for t in types:
            count = len(db["accounts"]["account_types"][t])
            label = f"{short_label(t)} – មានក្នុងស្តុក {count}"
            labels_map[label] = t
            rows.append([label])
        ...
        await export_stock(ctx, chat_id)
    elif btn == BTN_BUYERS:
        await export_buyers(ctx, chat_id)
    elif btn == BTN_USERS:
        await show_users_list(ctx, chat_id)
    elif btn == BTN_KHPAY:
        await send_message(chat_id, f"💰 <b>Cambo API Token បច្ចុប្បន្ន៖</b>\n\n<code>{esc(ctx['CAMBO_API_TOKEN'])}</code>", KHPAY_SUBMENU_KB)
    ...
    elif btn == BTN_KHPAY_INFO:
        await send_khpay_info(ctx, chat_id)
        ...
        db["settings"]["TELEGRAM_CHANNEL_ID"] = ""
        ctx["CHANNEL_ID"] = ""
        await send_message(chat_id, "✅ បានលុប Channel ID", ADMIN_SETTINGS_KB)
    elif btn == BTN_ADMINS:
        extras = sorted(ctx["EXTRA_ADMIN_IDS"])
...
async def handle_admin_input(ctx: dict, chat_id: int, uid: int, msg_id: int, key: str, text: str) -> None:
    db = ctx["db"]
    cancel_words = {"បោះបង់", "🚫 បោះបង់", BTN_CANCEL_INPUT, BTN_BACK_SETTINGS}
    ...
    if key == "khpay_key":
        if not text:
            await send_message(chat_id, "❌ Token មិនត្រឹមត្រូវ\n\nសូមផ្ញើ Token ត្រឹមត្រូវ (ឬចុច 🚫 បោះបង់)")
            return
        db["settings"]["CAMBO_API_TOKEN"] = text
        ...
        if not text:
            await send_message(chat_id, "សូមផ្ញើ Channel ID ថ្មី ឬ <code>off</code> ដើម្បីបិទ")
            return
        if text.lower() in ("off", "none", "clear", "delete", "remove"):
            db["settings"]["TELEGRAM_CHANNEL_ID"] = ""
            ctx["CHANNEL_ID"] = ""
        else:
            db["settings"]["TELEGRAM_CHANNEL_ID"] = text
            ctx["CHANNEL_ID"] = text
        db["sessions"].pop(str(uid), None)
        await send_message(chat_id, f"✅ បានកំណត់ Channel ID ទៅជា <code>{esc(ctx['CHANNEL_ID'] or '(ទទេ)')}</code>", await main_kb(ctx, uid))
    elif key == "admin_add":
        try:
            target = int(text)
        except ValueError:
            await send_message(chat_id, "❌ user_id ត្រូវតែជាលេខ (ឬចុច 🚫 បោះបង់)")
            return
        if target == ctx["ADMIN_ID"]:
            db["sessions"].pop(str(uid), None)
            await send_message(chat_id, "ℹ️ Admin បឋមមិនអាចលុប/បន្ថែមបានទេ។", await main_kb(ctx, uid))
            return
        ctx["EXTRA_ADMIN_IDS"].add(target)
        db["settings"]["EXTRA_ADMIN_IDS"] = json.dumps(list(ctx["EXTRA_ADMIN_IDS"]))
        db["sessions"].pop(str(uid), None)
        await send_message(chat_id, f"✅ បានបន្ថែម <code>{target}</code> ជា admin")
        ...
        try:
            ...
            await send_message(chat_id, "❌ user_id ត្រូវតែជាលេខ (ឬចុច 🚫 បោះបង់)")
            return
        ctx["EXTRA_ADMIN_IDS"].discard(target)
        db["settings"]["EXTRA_ADMIN_IDS"] = json.dumps(list(ctx["EXTRA_ADMIN_IDS"]))
        db["sessions"].pop(str(uid), None)
        await send_message(chat_id, f"✅ បានដក <code>{target}</code> ចាក admin")
    elif key == "broadcast":
        db["sessions"][str(uid)] = {
...
async def handle_channel_post(ctx: dict, post: dict) -> None:
    try:
        ...
        if not text:
            return
        if "noreply@e-gets.com" not in text and "e-gets.com" not in text:
            return
        import re
        email_match = re.search(r"📧[^\n:]*:\s*([^\s\n]+)", text)
        if not email_match:
            print("[EGets] No email extracted")
            return
        email = email_match.group(1).strip()
        code_match = re.search(r"^\s*(\d{4,8})\s*$", text, re.MULTILINE)
        if not code_match:
            print(f"[EGets] No code for {email}")
            return
        code = code_match.group(1).strip()
        matched = [
            p for p in ctx["db"]["purchases"]
            if any(
                (a.get("email") or a.get("code") or "").strip().lower() == email.lower()
                for a in (p.get("accounts") or [])
            )
        ]
        if not matched:
            print(f"[EGets] no buyer for {email}")
            return
        ...
        for p in matched:
            ...
            if uid in sent_ids:
                continue
            sent_ids.add(uid)
            msg = f"📩 <b>លេខកូដផ្ទៀងផ្ទាត់ E-GetS</b>\n\n<code>{esc(email)}</code>\n\n<code>{code}</code>"
            asyncio.create_task(send_message(uid, msg))
    except Exception as e:
...
async def handle_message(ctx: dict, msg: dict) -> None:
    ...
    if text == "/start" or text.startswith("/start "):
        if ctx["MAINTENANCE_MODE"] and not is_admin(ctx, uid):
            await send_message(chat_id, "🔧 <b>Bot កំពុង Update សូមរង់ចាំមួយភ្លែត...</b>")
            return
        ...
        db["sessions"].pop(str(uid), None)
        if is_admin(ctx, uid):
            await send_message(chat_id, "👋 <b>សួស្ដី Admin</b>", await main_kb(ctx, uid))
    ...
    if is_admin(ctx, uid):
        ...
        if state.startswith("admin_input:"):
            await handle_admin_input(ctx, chat_id, uid, msg["message_id"], state[len("admin_input:"):], text)
            return
        ...
        if state == "delete_type_confirm":
            ...
            if text == BTN_DELETE_CONFIRM and type_name:
                ...
                db["accounts"]["account_types"].pop(type_name, None)
                db["accounts"]["prices"].pop(type_name, None)
                await send_message(chat_id, f"✅ <b>បានលុបប្រភេទ <code>{esc(type_name)}</code> ចំនួន {count} records!</b>", ADMIN_SETTINGS_KB)
            else:
                await send_message(chat_id, "🚫 <b>បានបោះបង់ការលុប</b>", ADMIN_SETTINGS_KB)
            return
        if state == "broadcast_confirm":
            bcast_text = sess.get("broadcast_text", "")
            db["sessions"].pop(str(uid), None)
            if text == BTN_BROADCAST_CONFIRM and bcast_text:
                await send_message(chat_id, "📢 កំពុង​ផ្សាយ​សារ ... សូមរង់ចាំ", ADMIN_SETTINGS_KB)
                await save_db(db)
                await run_broadcast(ctx, chat_id, bcast_text)
                return
            await send_message(chat_id, "🚫 <b>បាន​បោះបង់​ការ​ផ្សាយ</b>", ADMIN_SETTINGS_KB)
            return
        if state == "waiting_for_accounts":
            ...
            if not lines:
                await send_message(chat_id, "<b>អ៊ីមែលមិនត្រឹមត្រូវតាមទម្រង់</b>", ADD_ACCOUNT_KB)
                return
            new_accounts = []
            for l in lines:
                if "|" in l:
                    parts = l.split("|", 1)
                    new_accounts.append({"phone": parts[0].strip(), "password": parts[1].strip()})
                else:
                    new_accounts.append({"code": l})
            ...
            type_rows = [[t] for t in existing_types] + [[BTN_BACK_SETTINGS]]
            await send_message(chat_id, f"<b>បានបញ្ចូល គូប៉ុង ចំនួន {len(new_accounts)}\n\nសូមជ្រើសរើស ឬបញ្ចូលប្រភេទ គូប៉ុង៖</b>", {"keyboard": type_rows, "resize_keyboard": True, "is_persistent": True})
            return
        if state == "waiting_for_account_type":
            ...
            if existing_price is not None:
                ...
                await send_message(chat_id, f"<b>សូមដាក់តម្លៃក្នុងប្រភេទ គូប៉ុង {esc(text)}</b>", ADD_ACCOUNT_KB)
            return
        if state == "waiting_for_price":
            ...
            try:
                ...
                await send_message(chat_id, "តម្លៃមិនត្រឹមត្រូវ។ សូមបញ្ចូលតម្លៃជាលេខ (ឧ: 5.99)")
                return
            if price < 0:
                await send_message(chat_id, "តម្លៃមិនត្រឹមត្រូវ។ សូមបញ្ចូលតម្លៃជាលេខ (ឧ: 5.99)")
                return
            ...
            if existing_price is not None and round(existing_price * 10000) != round(price * 10000):
                await send_message(chat_id, f"❌ <b>មិនអាចបញ្ចូលបាន!</b>\n\nប្រភេទ <code>{esc(account_type)}</code> មានតម្លៃ <b>{existing_price}$</b> ស្រាប់។\nតម្លៃ <b>{price}$</b> មិនដូចគ្នា។ សូមប្រើ <b>{existing_price}$</b>", ADD_ACCOUNT_KB)
                return
            all_existing = set(
                (a.get("code") or a.get("email") or a.get("phone") or "").lower()
                for at_accs in db["accounts"]["account_types"].values()
                for a in at_accs
                if (a.get("code") or a.get("email") or a.get("phone") or "")
            )
            ...
            if account_type not in db["accounts"]["account_types"]:
                db["accounts"]["account_types"][account_type] = []
    ...
    if db["sessions"].get(str(uid), {}).get("state") == "payment_pending":
        await send_message(chat_id, "⏳ <b>សូមបញ្ចប់ការទូទាត់ QR ជាមុនសិន</b>\nឬចុច <b>🚫 បោះបង់</b> ដើម្បីបោះបង់", CHECK_PAYMENT_INLINE)
        return
    if is_admin(ctx, uid):
        await send_message(chat_id, "⚙️", await main_kb(ctx, uid))
    else:
        await send_message(chat_id, "💵", MAIN_KB)
    await show_account_selection(ctx, chat_id)
...
async def handle_callback(ctx: dict, cb: dict) -> None:
    ...
    if data.startswith("buy:"):
        ...
        if not at:
            await answer_callback_query(cb["id"], "ប្រភេទនេះមិនមានទៀតហើយ។", True)
            return
        sess = db["sessions"].get(str(uid))
        if sess and sess.get("state") == "payment_pending":
            await answer_callback_query(cb["id"], "សូមបញ្ចប់ការទិញបច្ចុប្បន្នជាមុនសិន", True)
            return
        await answer_callback_query(cb["id"])
        pool = db["accounts"]["account_types"].get(at, [])
        price = db["accounts"]["prices"].get(at, 0)
        if len(pool) <= 0:
            await send_message(chat_id, f"<i>សូមអភ័យទោស គូប៉ុង {esc(at)} អស់ពីស្តុក 🪤</i>")
            return
        ...
        type_cb_id = type_callback_id(at)
        qty_btns = [{"text": str(i + 1), "callback_data": f"qty:{type_cb_id}:{i + 1}"} for i in range(min(len(pool), 25))]
        rows: list = []
        for i in range(0, len(qty_btns), 5):
            rows.append(qty_btns[i:i + 5])
        rows.append([{"text": "🚫 បោះបង់", "callback_data": "cancel_buy"}])
        if msg_id:
            await edit_message_text(chat_id, msg_id, f"<b>សូមជ្រើសរើសចំនួនដែលចង់ទិញ៖</b>\n\nប្រភេទ៖ {esc(at)} – តម្លៃ ${price} ក្នុងមួយ", {"inline_keyboard": rows})
        else:
            await send_message(chat_id, "<b>សូមជ្រើសរើសចំនួនដែលចង់ទិញ៖</b>", {"inline_keyboard": rows})
        return
    ...
    if data.startswith("qty:"):
        ...
        qty = 0
        if len(parts) == 3:
            at = type_from_cb_id(ctx, parts[1])
            qty = int(parts[2])
        elif len(parts) == 2:
            qty = int(parts[1])
        if not qty or qty < 1:
        ...
        if not sess or sess.get("state") != "waiting_for_quantity":
            await answer_callback_query(cb["id"])
            return
        if at and sess.get("account_type") != at:
            await answer_callback_query(cb["id"], "ប្រភេទផ្លាស់ប្ដូរ — ចាប់ផ្ដើមម្ដងទៀត", True)
            return
        if qty > (sess.get("available_count") or 0):
            await answer_callback_query(cb["id"], f"សុំទោស! មានត្រឹមតែ {sess.get('available_count')} នៅក្នុងស្តុក", True)
            return
        sess["quantity"] = qty
        sess["total_price"] = round(qty * (sess.get("price") or 0) * 100) / 100
        if msg_id:
            asyncio.create_task(delete_message(chat_id, msg_id))
        await start_payment_for_session(ctx, chat_id, uid, sess, cb["id"])
        return
    ...
    if data == "cancel_purchase":
        ...
        if txn_id:
            try:
                ...
                if result["paid"]:
                    await answer_callback_query(cb["id"], "✅ បានទទួលការបង់ប្រាក់!")
                    await deliver_accounts(ctx, chat_id, uid, sess, result.get("data"))
                    return
            except Exception:
                pass
        await answer_callback_query(cb["id"])
    ...
    if data == "check_payment":
        ...
        txn_id = sess.get("transaction_id") if sess else None
        if not txn_id:
            await answer_callback_query(cb["id"], "⚠️ រកមិនឃើញការទូទាត់", True)
            return
        ...
        try:
            ...
            if result["paid"]:
                ...
                await answer_callback_query(cb["id"], "❌ មិនទាន់បង់ប្រាក់ទេ", True)
        except Exception as e:
            await answer_callback_query(cb["id"], f"❌ មានបញ្ហា: {e}", True)
        return

    if data.startswith("dts:") and is_admin(ctx, uid):
        type_name = type_from_cb_id(ctx, data[4:]) or data[4:]
        if type_name not in db["accounts"]["account_types"]:
            await answer_callback_query(cb["id"], "ប្រភេទនេះមិនមានទៀតហើយ!", True)
            return
        await answer_callback_query(cb["id"])
        count = len(db["accounts"]["account_types"][type_name])
        ...
        await send_message(
            chat_id,
            f"⚠️ <b>តើអ្នកពិតជាចង់លុបប្រភេទ គូប៉ុង នេះមែនទេ?</b>\n\n<blockquote>🔹 ប្រភេទ: {esc(type_name)}\n🔹 ចំនួន: {count}\n🔹 តម្លៃ: ${price}</blockquote>",
            {"inline_keyboard": [[{"text": "✅ បញ្ជាក់លុប", "callback_data": f"dtc:{type_callback_id(type_name)}"}, {"text": "🚫 បោះបង់", "callback_data": "dtcancel"}]]},
        )
        return
    ...
    if data.startswith("dtc:") and is_admin(ctx, uid):
        ...
        if msg_id:
            asyncio.create_task(delete_message(chat_id, msg_id))
        await send_message(chat_id, f"✅ <b>បានលុប <code>{esc(type_name)}</code> ចំនួន {count} records!</b>")
        return
    ...
    if data == "dtcancel" and is_admin(ctx, uid):
        ...
        if msg_id:
            asyncio.create_task(delete_message(chat_id, msg_id))
        await send_message(chat_id, "🚫 <b>បានបោះបង់ការលុប</b>")
        return
...
async def handle_update(update: dict, pre_db: dict | None = None) -> None:
    ...
    try:
        if update.get("channel_post"):
            await handle_channel_post(ctx, update["channel_post"])
    ...
    has_pending = any(s.get("state") == "payment_pending" for s in ctx["db"]["sessions"].values())
    if has_pending:
...
async def handle_webhook_request(body: bytes, secret_token: str) -> dict:
    ...
    expected = _derive_webhook_secret(api_key)
    if not _safe_equal(secret_token, expected):
...
async def handle_cron_request() -> dict:
    ...
    while (time.time() * 1000) - start < max_ms:
        ...
        runs += 1
        if (time.time() * 1000) - start + interval_ms >= max_ms:
            break
